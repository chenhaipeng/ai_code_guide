...
| **progress.md** | 跨 session 执行摘要 | 轻量维护，必须短 | `templates/progress.md` |
| **README / CLAUDE.md / AGENTS.md** | 导航与约束（指向上面的源） | 改约束 / 入口时同步 | 顶层 |

> `specs/` / `plans/` 是"想清楚"和"拆清楚"的**思考脚手架**：写的目的是逼你想清楚（价值在创建时兑现）。经人类确认后供后续阶段消费，完成并确认后先把文件内内容状态改为 `Archived`，再移动到各自 archive，不长期维护。现状类信息回写到代码，不留在 spec 或 plan。详见 `reference/documentation-governance.md`。
...
| 场景 | 读什么 | 产出归到哪 |
| --- | --- | --- |
| 首次接手（新人 / AI） | `README` → `CLAUDE.md`/`AGENTS.md` → `progress.md` | — |
| 做技术 / 产品决策 | `decision.md`（历史）+ 代码（现状） | 新决策追加到 `decision.md` |
| 实现某功能 | 代码（现状）+ 相关 ADR（为什么） | 现状写进代码，决策进 `decision.md` |
| 跨 session 恢复 | `progress.md`（中断点）+ `CLAUDE.md` | 更新 `progress.md` |
...
│   ├── progress.md         ← 跨 session 执行摘要、任务进度、验证状态和下一步
...
- `docs/prompt.md` 保存目标项目运行态 prompt，由 AI 根据目标项目已有 prompt、项目约束、`docs/workflow.yaml.current`、进度摘要和当前相关 specs / plans / research 生成；已有则补充更新，不覆盖项目原有约定。
...
- `docs/progress.md` 保存跨 session 执行摘要，长任务中断、阶段切换和交付前后必须更新；当前 spec / plan item 以 `docs/workflow.yaml.current` 为准。
...
8. 每个 spec / plan 阶段完成后写入对应规格或计划，并更新 `docs/workflow.yaml`、`docs/progress.md` 和 `docs/prompt.md` 执行台账；进入执行层后写入验证报告或交付报告，并更新 `docs/progress.md` 和 `docs/prompt.md` 执行台账。
...
`docs/workflow.yaml` 默认只跟踪到 `90-implementation-plan`。阶段 8-11 属于执行层，按 implementation plan task、`docs/progress.md`、`docs/e2e/verify/` 和交付报告跟踪；`03-delivery-report` 是一次性交付总结，不进入 workflow，完成并确认后仍按 spec 归档到 `docs/superpowers/specs/archive/`。
...
- `docs/progress.md` 用于记录跨 session 执行摘要、已完成、进行中、未完成、阻塞、验证和下一步；当前 spec / plan item 以 `docs/workflow.yaml.current` 为准。
