...
>
> **角色定位**：本 spec 是设计快照（脚手架），用完归档。**接口契约真相以 OpenAPI 为准**（如 FastAPI `/openapi.json`，由代码生成）；本文件只记录页面-接口映射和关键契约决策（鉴权、幂等、SSE、错误码），**不逐接口手写请求/响应 schema**——那会迅速过时。详见 `reference/documentation-governance.md`。
