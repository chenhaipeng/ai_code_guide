...
4. 最后根据验收标准进行 e2e 验证，不达要求不要停止,验证结果写入 docs/e2e/verify/
5. 最后复核spec 和 spec 对应的 plan 状态的实现情况，如果实现完成，更新对应的spec和 spec 对应的 plan 状态，归档到 archive
```
...
```text
我的目标PandaWiki 源代码检索，能否分析解决用户提出的源代码问题，这个是最终的目标，按照按 opencode 的能力模型重做 PandaWiki 的源码检索工具层，先建一个 mian 和定义验收标准，然后拆解每个Phase，每个Phase 有依赖编号关系，请完成这些工作，最后根据验收标准进行 e2e 验证，不达要求不要停止
```