...
| **导航与状态（Where/Now）** | 东西在哪？当前 spec / plan item 是什么？prompt 阶段走到哪？ | `README`、`workflow.yaml`、`prompt.md` 执行台账、`CLAUDE.md`/`AGENTS.md` | **轻量维护**：必须短，长了就不更新了 |
...
- 因此 specs 和 plans 在**经人类确认、被后续阶段消费、对应实现/验收验证通过**后，先把文件内内容状态改为 `Archived`，再移动到各自 archive（git 历史保留），不要试图长期维护它们与代码一致——那是徒劳。
- 特别是 `30-data-design`、`40-api-and-pages` 这类描述"现状"的 spec：现状的真相在代码（ORM / OpenAPI），手写必过时。

## 主流程状态机

`docs/workflow.yaml.current` 是当前 spec / plan item 的权威来源；阶段诊断表和 `docs/prompt.md` 执行台账只能解释、展示或辅助纠偏，不能各自形成另一套当前 item 真相。
...
人类 review 是强制 gate：`review → ready`、implementation plan `review → ready`、`verified → archived` 必须经人类确认。归档前必须先把文件内内容状态改为 `Archived`，再按 `docs/workflow.yaml` 的 `archive_path` 移动文件，并同步更新 `docs/workflow.yaml` 和 `docs/prompt.md` 执行台账。
...
3. **workflow.yaml** — 当前 spec / plan item、生命周期、依赖和归档路径
4. **prompt.md 执行台账** — prompt 阶段执行状态、跳过原因、阻塞和当前推荐 prompt

加上 **CLAUDE.md / AGENTS.md**（AI 行为约束）。其余要么是代码（接口/数据/行为），要么是写完即弃的脚手架（specs / research / plans），要么是验证证据（`docs/e2e/verify/`）。

## 五条铁律

1. **"现状"类一旦手写，就注定游离。** 接口/数据/行为文档，真相只能是代码或代码生成物（OpenAPI / ORM+迁移 / 测试）。
2. **"决策"类写完即封存。** ADR 是 append-only 历史账本，不更新。
3. **唯一需要手动维护的是导航状态，且必须轻。** 长了就不更新了；实现任务放 plan task，验证证据放 verify 报告，不另建进度摘要。
4. **写文档的价值在"写"的动作，不在维护产物。** 早期 spec/设计/计划有必要写，但不要在刚写完时归档；确认、消费、验证后先改内容状态为 `Archived`，再移动到对应 archive。
...
- ❌ README / prompt 台账堆砌全部历史细节，压垮维护意愿
- ❌ 归档文档留在主索引，误导新人
- ❌ 内容状态（Draft / AI Extracted / Human Confirmed）和 workflow 状态（review / ready / archived）混用，导致 review gate 失效

## 新项目怎么套用

1. 从模板建 `CLAUDE.md` / `AGENTS.md`（约束）、`decision.md`（空 ADR）、`workflow.yaml`（状态机）、`prompt.md`（运行态 prompt 和执行台账）、`README.md`（导航）。
...
4. 每完成一个 workflow item，对应 spec 先改内容状态为 `Archived` 再进 `docs/superpowers/specs/archive/`；对应 plan 先改内容状态为 `Archived` 再进 `docs/superpowers/plans/archive/`（git 历史是最终 archive）。
