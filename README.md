...
| 文档 | 定位 | 适合 |
|---|---|---|
| [Coding Template](coding_template/README.md) | AI 产品工程模板体系：从 idea 到规格、实施、E2E 验收和交付报告的完整上下文系统 | 启动新产品、新模块或需要 AI 深度参与的软件项目 |
...
| 文档 | 定位 | 适合 |
|---|---|---|
| [系统级 Bug 狩猎提示词](promt/系统级%20Bug%20狩猎提示词.md) | 针对 AI 工具的系统级 bug 定位 prompt 模板 | 利用 AI 工具做深度 bug 定位和根因分析 |
...
| 流程阶段 | Superpowers | Spec Kit | OpenSpec | OpenCode + omo | oh-my-claudecode | oh-my-codex |
|---|---|---|---|---|---|---|
| **需求澄清** | brainstorming（自动） | `/speckit.clarify`（手动） | 内嵌在 `/opsx:propose` | Tab 切 Prometheus（手动） | `/deep-interview`（手动） | `/plan`（手动） |
| **规划** | writing-plans（自动） | `/speckit.plan`（手动） | `/opsx:propose`（手动） | Prometheus 三剑客（自动） | `/ralplan`（手动） | `/ralplan`（手动） |
| **执行** | executing-plans（自动） | `/speckit.implement`（手动） | `/opsx:apply`（手动） | `/start-work` 或 `ulw:`（手动） | `/team` 或 `/autopilot`（手动） | `/ultrawork` 或 `/ralph`（手动） |
| **测试** | TDD（自动，强制） | Constitution 约束（plan 阶段） | 无对应 | 无对应 | `tdd` 关键字（手动） | `/tdd`（手动） |
| **验证** | verification（自动） | `/speckit.analyze`（手动） | `/opsx:verify`（手动） | `review-work` skill（手动） | `/ralph` 循环（自动） | `/ultraqa` 循环（自动） |
| **Review** | requesting-review（自动） | `/speckit.checklist`（手动） | `/opsx:archive`（手动） | `@oracle`（手动） | `/ask codex`（手动） | `/code-review`（手动） |
| **人类介入** | brainstorm 环节 | 全程手动 | 全程手动 | `/start-work` 手动 | 手动触发 | 手动触发 |
| **自动化程度** | **全自动** | 全手动 | 全手动 | 半自动 | 半自动 | 半自动 |
