...
### 3.1 Hash-Anchored Edit Tool（最重要）`自动`
...
| `unspecified-high` | anthropic/claude-opus-4-6 (max) | 其他任务，高投入 |
...
### 3.3 11 个专业化 Agent（核心团队）`手动切换 / 自动调度`

> ⚠️ 修正：网上很多文档列的 agent（如 `frontend-ui-ux-engineer`、`document-writer`）**不存在**。以下是 features.md 实测列表。

**核心 agents（按 Tab 切换顺序）：**

| 序号 | Tab 显示名 | Agent | 默认模型 | 触发方式 |
| --- | --- | --- | --- | --- |
| 1 | Sisyphus - Ultraworker | **Sisyphus** | claude-opus-4-6 | 按 Tab 手动切换；`ulw:` 自动激活 |
| 2 | Hephaestus - Deep Agent | **Hephaestus** | gpt-5.4 | 按 Tab 手动切换 |
| 3 | Prometheus - Plan Builder | **Prometheus** | claude-opus-4-6 | 按 Tab 手动切换，用于采访式规划 |
| 4 | Atlas - Plan Executor | **Atlas** | claude-sonnet-4-6 | `/start-work` 手动触发后自动接管 |

**辅助 agents（通过 `@` 显式调用）：**

| Agent | 默认模型 | 触发方式 | 用途 |
| --- | --- | --- | --- |
| **Oracle** | gpt-5.4 | `@oracle` 手动 | 架构决策、code review、调试（**只读**） |
| **Librarian** | minimax-m2.7 | `@librarian` 手动 | 多仓库分析、文档查找（只读） |
| **Explore** | grok-code-fast-1 | `@explore` 手动 | 快速代码搜索 + 上下文 grep（只读） |
| **Multimodal-Looker** | gpt-5.4 | `@multimodal-looker` 手动 | 分析 PDF / 图片 / 图表（只读） |
| **Metis** | claude-opus-4-6 | `@metis` 手动 / Prometheus 规划时**自动**介入 | 计划顾问，识别隐藏意图和模糊点 |
| **Momus** | gpt-5.4 | `@momus` 手动 / Prometheus 规划时**自动**介入 | 计划评审者，验证清晰度/可验证性 |
| **Sisyphus-Junior** | category 决定 | Sisyphus 按 category **自动**派发 | category 派发的执行者 |
...
### 3.5 Prometheus 三剑客（采访式规划）`手动触发`

**规划入口：** 按 `Tab` 键切换到 Prometheus agent，然后输入任务描述
**执行入口：** `/start-work`（规划完成后）

> ⚠️ **触发方式澄清**：Prometheus 不是通过 `@plan` 调用的，它是核心 agent 之一（Tab 顺序第 3 个）。在 TUI 输入框中**按 Tab 键**切换到 Prometheus，然后直接描述任务即可。`@` 列表中**不会出现 Prometheus**——只有辅助 agent（Oracle、Librarian、Explore 等）和规划辅助（Metis、Momus）在 `@` 列表中。

- **Prometheus**：像真正的工程师那样**采访你**，识别 scope 和模糊点，输出可验证的 plan
- **Metis**（`@metis` 可显式调用）：在 plan 落地前自动做前置分析，识别 AI 容易踩坑的地方
- **Momus**（`@momus` 可显式调用）：在 Metis 之后自动评审 plan 的清晰度、可验证性、完整性

流程：切换到 Prometheus → 描述任务 →（Prometheus 采访 + Metis 分析 + Momus 评审）→ `/start-work`（Atlas 调度执行）

这是 omo 区别于普通"先 plan 后执行"模式的核心，避免 prompt-and-pray。

### 3.6 Hierarchical AGENTS.md（`/init-deep`）`手动触发`

> ⚠️ `/init-deep` 是 omo 提供的命令，不是 OpenCode 内置。只装 OpenCode 没有 omo 插件时此命令不存在。OpenCode 内置只有 `/init`（生成单个项目级 AGENTS.md）。
...
### 3.7 Background Agents + Tmux 集成 `自动调度`
...
### 3.12 LSP + AST-Grep 工具链 `自动`（Agent 调用时自动使用）
...
### 3.13 Skill-Embedded MCP `手动触发`（通过 skill 名称激活）

Skills 可以**自带 MCP server**，按需启动、scoped to task、用完即关。避免常驻 MCP 吃 context budget。

内置 skills：`playwright`（浏览器自动化）、`agent-browser`（Agent Browser CLI 浏览器自动化）、`dev-browser`（有状态浏览器脚本）、`git-master`（原子提交、rebase 手术）、`frontend-ui-ux`（设计优先 UI）、`review-work`（5 并行 sub-agent 代码审查）、`ai-slop-remover`（清除 AI 代码气味）。
...
> ⚠️ **常见混淆澄清**：
>
> 1. **`/init-deep`（层级化 AGENTS.md）是 omo 提供的，不是 OpenCode 内置。** 只装 OpenCode 没有 omo 插件时，`/init-deep` 不存在。OpenCode 内置只有 `/init`（生成单个项目级 AGENTS.md）。
>
> 2. **Tab 切换 Plan 模式 ≠ Prometheus 规划。** 按 Tab 切到 OpenCode 内置的 Plan/Build 模式只是简单切换；要使用 Prometheus 三剑客深度规划流程，需安装 omo 后继续按 Tab 切到 Prometheus agent（核心 agent 第 3 个）。两者完全不同。
...
| Tab 切换到 Prometheus + 输入任务 | — | 规划入口（Prometheus 采访式规划） |
...
|---|---|---|
| 核心流程 | **全自动**，你说需求它自己走 | **大部分手动触发**（Tab 切 Prometheus、`ulw:`、`@oracle`） |
| 设计哲学 | 让你不管流程，AI 全权负责 | 给你控制权，你来调度 agent |
| 典型用法 | "我想做 X" → AI 自动 brainstorm → plan → TDD | Tab 切 Prometheus → `/start-work` → `ulw:` → `@oracle review` |
...
### 5.2 Prometheus + `/start-work`：采访式规划与执行

> **注意**：这是 omo 提供的功能（需安装 omo 插件）。Prometheus 通过按 `Tab` 键切换到（核心 agent 第 3 个），不是通过 `@plan` 触发。OpenCode 内置的 Tab Plan 模式只是简单的计划/执行切换，不包含 Prometheus 三剑客流程。
...
```jsonc
{
  "background_task": {
    ...
    "modelConcurrency": {
      "anthropic/claude-opus-4-6": 2  // Opus 严控
    }
  }
}
```
...
| 模型 | 评价 |
|---|---|
| **Claude Opus 4.6** | 最佳整体体验，prompt 针对它优化 |
...
```jsonc
{
  "categories": {
    "visual-engineering": {
      "model": "anthropic/claude-opus-4-6"  // 覆盖默认的 gemini-3.1-pro
    }
  }
}
```
...
是简单任务吗？
  ...
  └─ NO  → 解释上下文很麻烦吗？
              ...
              └─ NO  → 需要精确、可验证的执行吗？
                         └─ YES → Tab 切 Prometheus 规划 → /start-work 执行
                         └─ NO  → 直接用 ulw
```
...
| 复杂且要精确 | Tab 切 Prometheus → `/start-work` | 多步骤、需要验证的编排执行 |
...
|---|---|---|
| 触发 | `ulw` / `ultrawork` | Tab 切 Prometheus → `/start-work` |
...
/init-deep                              ← 第一步，建立层级化知识骨架
    ↓
[Tab 切到 Prometheus] "build X feature"     ← Prometheus 采访式规划
...
@explore 分析项目结构和相关模块          ← 快速建图（grok-code-fast-1）
    ↓
[Tab 切到 Prometheus] "add X feature to module Y"  ← Prometheus 进入采访模式
...
```
[Tab 切到 Prometheus] "refactor the authentication module"  ← Prometheus 采访，应用 Safety 策略
...
```
[Tab 切到 Prometheus] "design architecture for X"   ← Prometheus 应用 Strategic 策略
...
| 快速修复 | 直接描述 | 完成 |
| 复杂但懒得解释 | `ulw <描述>` | 让它跑 |
| 复杂且要精确 | Tab 切 Prometheus + `"<描述>"` | `/start-work` |
| 长时间任务 | `/ulw-loop` 或 `/ralph-loop` | `/cancel-ralph` 停止 |
| 新项目 | `/init-deep` → Tab 切 Prometheus | `/start-work` |
| 重构 | `/refactor "<描述>"` | `review-work` skill |
| Code Review | `review-work` skill | `ulw` 修复问题 |
| 架构设计 | Tab 切 Prometheus + `@oracle` + `@librarian` | `/start-work` |
| 结束会话 | `/handoff` | 下次自动恢复 |
| 简单 Bug | 直接描述 | 完成 |
...
| Plan 模式 | 内置 | Prometheus 三剑客（Tab 切换）+ `/start-work` 执行（更强） |
...
|---|---|---|
| `ultrawork` | `ulw` | 激活全功能编排 |
| Tab 切到 Prometheus + 任务描述 | - | 规划入口（Prometheus 采访式规划） |
| `/start-work` | - | 执行入口（读取 plan 后 Atlas 调度 agent） |
| `/ulw-loop` | - | Ralph 循环，不到 100% 不停 |
...
lsp_diagnostics / lsp_prepare_rename / lsp_symbols
...
## 13. Superpowers 工作流提示指南

> 本节回答：**"如果我要使用 Superpowers 的流程来开发，应该如何提示？"**
>
> Superpowers 是 omo 内置的一套纪律化开发 skills 体系（brainstorming → plan → TDD → implement → review → verify），通过你的提示词自动激活对应 skill，无需手动配置。
...
| `using-git-worktrees` | "在隔离环境开发"、"feature branch 隔离" | 创建隔离的 git worktree 进行开发 |
...
3. **TDD** — 先写测试，再写实现代码
4. **implementing** — 按计划执行，自动选 category 和 skill
...
### 13.5 Superpowers 与 omo 原生流程的关系
