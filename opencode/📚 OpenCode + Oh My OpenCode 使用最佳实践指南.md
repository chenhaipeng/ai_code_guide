...
>
> 2. **Tab 切换 Plan 模式 ≠ `@plan`。** 按 Tab 键切换的是 OpenCode 内置的简单 Plan/Build 模式；`@plan` 是 omo 提供的 Prometheus 三剑客深度规划流程（采访式、多 Agent 协作）。两者完全不同，没装 omo 时 `@plan` 不会生效。
...
> **注意**：`@plan` 是 omo 提供的功能（需安装 omo 插件）。OpenCode 内置的 Tab Plan 模式只是简单的计划/执行切换，不包含 Prometheus 三剑客流程。
