...
- **Metis**：在 plan 落地前做前置分析，识别 AI 容易踩坑的地方
- **Momus**：评审 plan 的清晰度、可验证性、完整性
...
| 命令 | 作用 |
|---|---|
| `/start-work` | 采访式规划（Prometheus 三剑客） |
...
|---|---|---|
| 核心流程 | **全自动**，你说需求它自己走 | **大部分手动触发**（`/start-work`、`ulw:`、`@oracle`） |
| 设计哲学 | 让你不管流程，AI 全权负责 | 给你控制权，你来调度 agent |
| 典型用法 | "我想做 X" → AI 自动 brainstorm → plan → TDD | `/start-work` → 规划 → `ulw: 执行` → `@oracle review` |
...
### 8.1 新系统开发（Greenfield）

**关键动作（不是模板）：**

1. `/init-deep` —— 给 agent 建立项目认知骨架
2. `/start-work` —— Prometheus 采访式规划，避免一上来就写代码
3. `ultrawork` 执行实际实现，让 Sisyphus 编排
4. `@oracle` 做架构 review（只读，无副作用）

**反模式：** 上来就 `ultrawork: 帮我实现整个用户系统`。先用 `/start-work` 把 scope 撑开。

### 8.2 既有系统加功能（Brownfield）

**关键动作：**

1. `@explore 分析项目结构` —— 让 explore（grok-code-fast-1）快速建图
2. `@oracle 这个改动会影响哪些模块？` —— 影响分析
3. `/start-work` 做兼容性规划
4. `ultrawork` 执行
5. `@oracle review` 终审
...
1. `@explore` 列出待重构代码的所有调用点
2. `@oracle` 评估重构风险
3. 用 `lsp_rename` / `ast_grep_replace` 做**结构化**重构（不要让 agent 手动重写大段代码）
4. 每步小提交 + 跑测试
